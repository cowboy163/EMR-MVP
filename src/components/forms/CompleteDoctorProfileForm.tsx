...
import { DoctorProfileSchema, doctorProfileSchema, memberProfileSchema } from '@/lib/schemas/registerSchema'
...
export default function CompleteDoctorProfileForm() {
    // two steps to register
    const [activeStep, setActiveStep] = useState(0);
    const currentValidationSchema = stepSchemas[activeStep];

    // form provider methods
    const methods = useForm<DoctorProfileSchema>({
    ...
    const onSubmit = async () => {
        const result = await completeSocialLoginProfile(getValues(), 'DOCTOR');
        if (result.status === "success") {
            signIn(result.data, {
                callbackUrl: getBaseLinkBasedOnRolePlatform('DOCTOR')
            })
        }
    }
